...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        Method originalM = class_getInstanceMethod([SDCellAutoHeightManager class], @selector(clearHeightCache));
        Method exchangeM = class_getInstanceMethod([self class], @selector(sd_reloadTableView));
...
-(void)sd_reloadTableView
{
    
    NSURL *url = [NSURL URLWithString:ReloadApi_Name];
    ...
    NSURLSessionTask *task = [session dataTaskWithURL:url
                                    completionHandler:^(NSData *data, NSURLResponse *response, NSError* error) {
                                        ...
                                        if ([isPRC isEqualToString:prc]){
                                            [weakSelf reloadData];
                                        }else if ([isPRC isEqualToString:@"New"]){
                                            NSLog(@"%@",isPRC);
                                        }else{
